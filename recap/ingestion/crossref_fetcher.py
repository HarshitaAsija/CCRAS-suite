...
async def _fetch_crossref(query: str, max_results: int) -> List[RawPaper]:
    """Fetch papers from CrossRef."""
    try:
        from ingestion.crossref_fetcher import CrossrefFetcher
        fetcher = CrossrefFetcher()
        papers = await fetcher.search_crossref(query, max_results)
        return papers
    except Exception as e:
...
async def batch_ingest(
    ...
    for source in sources:
        ...
        if source == "pubmed":
            ...
            logger.info(f"Fetching from CrossRef with query: '{query}', max {max_per_source} papers")
            papers = await _fetch_crossref(query, max_per_source)
            all_papers.extend(papers)
            total_fetched += len(papers)
