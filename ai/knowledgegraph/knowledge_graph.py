...
LLM_PROVIDER    = os.getenv("LLM_PROVIDER", "qwen2.5:7b").lower()   # "gemini" | "ollama"
...
Group these keywords into canonical concepts. Merge synonyms.
...
def normalize_concepts_with_llm(query: str, keywords: list[str]) -> dict:
    ...
    try:
        ...
        logger.info(f"[{LLM_PROVIDER}] normalized {len(unique_kws)} keywords → "
                    f"{len(set(data['canonical_map'].values()))} concepts, {n_rels} typed relationships")
        if n_rels == 0:
...
def build_graph(
    ...
    # canonical_map can legally contain far more distinct concepts than
    # MAX_CONCEPTS — the LLM prompt can only ask nicely for a limit, so we
    # enforce it here. Always keep the query concept and anything the LLM
    # explicitly related; fill remaining budget by paper-count (weight).
    ...
    for a, b, w in cooccur_candidates:
        if cooccur_fanout[a] >= MAX_COOCCUR_EDGES_PER_NODE or cooccur_fanout[b] >= MAX_COOCCUR_EDGES_PER_NODE:
            continue
    ...
    # Drop orphan nodes (no edges at all) — these are what were flying off
    # to random positions and distorting the force-directed layout, since
    # cose has nothing to anchor them to.
    connected = {query_concept}
    for (a, b) in edges:
        connected.add(a)
        connected.add(b)
    if len(connected) > 1:
        dropped = set(nodes) - connected
        if dropped:
            logger.info(f"Dropping {len(dropped)} orphan concept(s) with no relationship edges")
        nodes = {c: meta for c, meta in nodes.items() if c in connected}
...
class KnowledgeGraphPipeline:
    ...
    def run(self, query: str, user_id: str = "default") -> dict:
        ...
        # Step 3 — collect all raw keywords (DB column + text-mined)
        all_kws = [query]
