...
function getBaseUrl(): string {
  if (typeof window !== "undefined") {
    return process.env.NEXT_PUBLIC_API_URL ? `${process.env.NEXT_PUBLIC_API_URL}/api/v1` : `http://${window.location.hostname === "localhost" ? "127.0.0.1" : window.location.hostname}:8002/api/v1`;
  }
  return process.env.NEXT_PUBLIC_API_URL ? `${process.env.NEXT_PUBLIC_API_URL}/api/v1` : "http://127.0.0.1:8002/api/v1";
}
...
const RISHI_BASE_URL = typeof window !== "undefined"
  ? process.env.NEXT_PUBLIC_GAPS_API_URL || `http://${window.location.hostname === "localhost" ? "127.0.0.1" : window.location.hostname}:8001`
  : process.env.NEXT_PUBLIC_GAPS_API_URL || "http://127.0.0.1:8001";
